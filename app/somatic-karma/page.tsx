...
export default function SomaticKarmaPage() {
  return <>
    ...
    <section className="rooms"><div className="shell"><div className="section-heading"><div><p className="eyebrow">DIE DIGITALEN RÄUME</p><h2>Wähle deinen Zugang zur inneren Landkarte.</h2></div></div><div className="room-grid">{rooms.map(room=><a className="room-card" href={room.href} key={room.title}><img src={room.image} alt="" /><div><span>{room.label}</span><h3>{room.title}</h3><p>{room.text}</p><b>Öffnen</b></div></a>)}</div></div></section>
